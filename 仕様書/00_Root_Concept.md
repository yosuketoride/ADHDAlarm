...
- アラーム削除時は `.caf` も必ず削除する（`VoiceFileGenerator.deleteAudio`）
- 読み上げテンプレート: 「お時間です。あと{X}分で{タイトル}のご予定ですよ。」
...
**鉄則**: コア機能（アラームが鳴る）は**絶対に**課金壁にしない。

`SubscriptionTier` を必ずチェックしてから PRO 機能を出し分けること:
```swift
