...
- 30分以内の複数予定: グループ表示（アニメなし）
  注: 現行実装は `nextAlarmCard` の色変化・一覧表示中心で、旧チェック項目の「青い円形リング」「赤パルス」「複数予定グループ表示」前提は対象外
- [x] 今日0件（全完了）: 「🎉 お疲れ様！」
- [x] 今日0件（スキップ含む）: 「🍵 今日は無理せず休もう」
- [x] 今日0件（最初から予定なし）: 「🌸 のんびりしてね」
- 明日の予定: 区切り線+グレーで常時2件表示（折りたたみなし・0件でもスクロール下固定）
  注: 現行実装は `upcomingDefaultCount = 2` と区切り帯「ここから明日以降」はあるが、件数超過時の展開/折りたたみUIも残っているため旧固定表示前提は対象外
- [x] オフライン: 家族モード上部に黄色バナー
  注: 現仕様では当事者ホームではなく `FamilyHomeView` 上部に表示する
- FAB: 「🎤 予定を追加」ラベル常時表示
  注: 現行 `micFAB` は通常サイズ時のみ「予定を追加」ラベルを表示し、アクセシビリティサイズではアイコン主体の拡大ボタンに切り替わる
- [x] XP: 完了+10 / スキップ+3 正確に加算（アプリ起動XPなし）
- [x] XP: 予定追加XPは完了時のみ付与
- [x] XP: 1日上限50XP
- [x] ふくろう: 数字UI（Lv.7等）が存在しないこと
...
- 「今回はパス」は長押し1.5秒で発動（タップでは発動しない）
  注: 現行実装は全幅タップの「とめる」ボタン + `confirmationDialog` によるスキップ確認へ移行しているため対象外
- SOSバー: `alarm.fireDate` から正しく経過時間を計算
  注: 現行 `RingingView` の SOS UI は送信成功/失敗バナー中心で、旧「経過時間表示つきSOSバー」前提の実装は確認できないため対象外
- [r] イヤホン抜け: AVAudioPlayer停止 + AlarmManager.cancel() 両方呼ばれること
- [r] イヤホン抜け: RingingViewは閉じずに維持されること
- 変動比率: スペシャル(5%)が偏りなく出現すること
  注: 現行のフクロウタップ演出は `Int.random(in: 0..<100)` を使った 40% / 30% / 20% / 10% 構成で、旧「スペシャル5%」前提は対象外
- スライド完了: Y軸80ptまでのズレがあってもドラッグが継続されること（斜め操作対応）
- スライド完了: 長押し2秒でも完了発動すること（フォールバック）
- スライド完了: 「または長押しで完了」テキストが常時表示されること
  注: 現行実装はスライド完了UIではなく全幅タップボタンに置き換わっているため対象外
- [x] owlAmber背景のボタンのテキスト色が黒（#000000）であること（コントラスト8.3:1）
- [x] owlAmber背景に白テキストが1箇所も存在しないこと（ビジュアル検査）
- [x] `UIAccessibility.isReduceMotionEnabled = true` の時: RingingView の波紋・上下バウンス演出が停止すること
- [x] `UIAccessibility.isReduceMotionEnabled = true` の時: RingingView は短いフェードイン中心で表示されること
- ~~`CircularCountdownView` に `.dynamicTypeSize(...DynamicTypeSize.accessibility2)` が適用されていること~~ → **v16廃止（P-2-4）**
- [x] **[v16 P-2-4]** accessibility3以上の時: 円形リングが非表示になり巨大数字テキストのみが表示されること
- [x] システムフォントを最大設定にした時、カウントダウン数字 > イベントタイトルの階層が維持されること
- ~~完了/パス後3秒以内: Undoスナックバーが表示されること~~ → **v16廃止（P-2-1）**
- **[v16 P-2-1]** 完了/パス後: DismissSheet（ハーフシート）が表示されること
  注: 現行実装は `DismissSheet` ではなく `PersonHomeView` 側の `undoBanner` / `completeUndoBanner` に置き換わっているため対象外
- **[v16 P-2-1]** DismissSheet に「取り消す」ボタンが表示されること
  注: 現行実装は `DismissSheet` ではなく `undoBanner` / `completeUndoBanner` の「もとに戻す」ボタンに置き換わっているため対象外
- **[v16 P-2-1]** DismissSheet が10秒で自動閉じすること
  注: 現行実装は `DismissSheet` ベースではなく、同文言の自動閉じロジックも確認できないため対象外
...
### 家族モード
- [x] 🔒 機能（履歴・SOS）: タップで FamilyPaywallView が表示されること
- [x] 当日の完了/スキップステータス（✓/❌）: 無料版でも表示されること（壁なし）
...
- [x] 「あとで」でスキップ可能
- [x] 無料版でロック機能にタップするとペイウォールが出ること
- [x] 親の初回✓✓到着時: 無料ユーザーにコンテキスチュアルバナーが出ること（2回目以降は出ない）
  注: 現行実装では `FamilyDashboardTab` 上部に「✓✓ が届きました」バナーを表示し、`UserDefaults` で端末ごとに1回だけ出す
- [2] レビュー依頼: 月次サマリー（10回完了以上）または家族に初回✓✓後のみ発火すること
- [x] レビュー依頼: 起動直後・エラー後・スキップ後・ペイウォール後に発火しないこと
...
- [x] RingingView: スライドではなく「薬を飲んだ」タップボタン（ComponentSize.actionGiant = 72pt）が表示されること
...
- MagicDemoView（スライドUI）の .selection / .success タイミングが正しいこと
  注: 現行 `MagicDemoView` は `RingingView` 呼び出しと確認ダイアログ中心で、旧スライドUI前提は対象外

### 家族モード（追加項目）
- 予定送信: 時間プリセット（朝/昼/夜）ボタンが3つ表示されること
  注: 現行 `FamilySendTab` は 朝/昼/夜 ではなく `今から / 15分後 / 30分後 / 1時間後 / カスタム` の選択式のため対象外
- 重複検知: 警告バナーではなく「上書きする/やめる」2択が表示されること
- 重複検知: 選択前は送信ボタンがブロックされること（強行突破不可）
  注: 現行の家族送信GUI (`FamilyInputView`) では重複検知自体をまだ確認できず、旧重複解決フロー前提は対象外
...
- [x] `isLowPowerModeEnabled = true` の時: RingingView の装飾アニメーションがスキップされること
...
- 15分超過の場合: ふくろうトースト「さっきのお知らせ、時間が経っていたのでお休みにしておきました」が表示されること
  注: 現行コードベースでは `.missed` 保存ロジックはあるが、この文言のトースト表示までは確認できず、旧文言固定前提は対象外
...
- 同意確認画面に「できること / できないこと」が両方明記されていること
  注: 現行コードベースでは専用の同意確認画面をまだ確認できず、`FamilyPairingView` のコード入力フロー中心のため対象外
...
- [x] 設定画面のラベルにIT用語が1つも使われていないこと（翻訳表に従っていること）

### オンボーディング（追加項目）
- [r] MagicDemoでアラームが鳴った後、Stage 2 RingingView まで通り抜けること
- RingingViewで完了タップ → OwlCelebrationOverlay が表示されること
  注: 現行実装は `OwlCelebrationOverlay` ではなく Toast による「よくできました！ ⭐️ +10ポイント」表示へ置き換わっているため対象外
- [x] MagicDemoの +10 XP が本物のXPとして加算されること

### プライバシー
- [r] Stage 1（OSアラーム）: タイトルが「🦉 ふくろうからのお知らせ」
- [x] Stage 2（RingingView）: タイトルが表示されること
- [x] 通知バナー: 予定タイトルが含まれないこと
...
- [x] EventRow左端に絵文字アイコン（.title2相当・Dynamic Type追従）が表示されること
- [x] タイトルから絵文字が自動推定されること（💊薬 / 🏥病院 / 🛒買い物 等）
- [x] 推定不能の場合: デフォルト 📌 が表示されること（空欄にならない）
- [x] 完了状態: 絵文字が opacity 0.4 でグレーアウトされること

### EventRow インタラクション（v2変更）
> ゴミ箱ボタン廃止・ジェスチャー操作に統一
- 右スワイプ: 緑「完了」ボタンが表示されること（allowsFullSwipe: false で誤操作防止）
  注: 現行実装は `swipeActions` ではなく `confirmationDialog` と長押し操作中心の構成に寄っているため対象外
- 左スワイプ: 赤「削除」ボタンが表示されること（タップで confirmationDialog を表示）
  注: 現行 `EventRow` は右端の削除ボタンと長押し操作メニューを持ち、`swipeActions` 前提は対象外
...
- 行内ヒント: 「長押しで完了 ・ 左スワイプで削除」テキストが未完了行に表示されること
  注: 現行 `EventRow` / `PersonHomeView` ではこのヒント文言は確認できず、旧ジェスチャー前提のため対象外
- [x] 完了済み行: 右端にチェックマークアイコンのみ表示（ボタンなし）

### 連携解除UI（v11追加）
- [2] 設定 → つながっている家族の設定: ペアリング相手の名前と連携日が表示されること
- 「連携を解除する」ボタン（.statusDanger色）が表示されること
  注: 現行実装は `.statusDanger` のテキストリンクではなく、`bordered`/赤系ボタンや別UIに分かれているため対象外
- タップ → 確認ダイアログ（ActionSheet）が表示されること
  注: 現行の家族連携解除導線はビューごとに実装が分かれ、即時解除や `.alert` / `confirmationDialog` ベースが混在しているため対象外
- [2] 確定後: `family_links.is_active = false` が Supabase に書き込まれること
- [2] 確定後: 相手側に解除Push通知が送信されること
- [x] 確定後: ローカルの familyLinkId / familyChildLinkIds がクリアされること
...
- バッチ通知（30分以内複数スキップ）: 「体調を確認してあげてください ☕️」が含まれること
  注: 現行コードベースではこの通知本文テンプレートやバッチ文言を確認できず、旧通知コピー前提は対象外
...
- クリーンアップ中にUIがブロックされないこと（バックグラウンドスレッド）
  注: 現時点では専用の `.caf` クリーンアップ実装や実行スケジューラをコードベース内でまだ確認できず、旧クリーンアップ仕様前提は対象外

### DatePickerアコーディオン（v11追加）
- [x] FamilySendTab: DatePicker がデフォルトで非表示（折りたたみ状態）なこと
- [x] 「⚙️ 時間を細かく設定する」タップ → DatePicker がアニメーションで展開されること
  注: 現行の該当導線は `FamilySendTab` ではなく `PersonManualInputView` 側にある
- [x] プリセット（朝/昼/夜）タップ → 日時が即時設定され、アコーディオンは閉じたままなこと
...
- App Group UserDefaults 経由で owlName/owlState が正しく読み取られること
  注: 現行ウィジェットは App Group から `owl_name` / `owl_xp` を読み、感情状態は `owlState` 保存値ではなく残り時間から導出しているため旧仕様前提は対象外
...
- [x] 「本当に解除する」ボタンが .destructive スタイルで表示されること
- ボタンが `.buttonStyle(.borderedProminent)` になっていないこと（ビジュアル確認）
  注: 現行の連携解除UIは `SOSPairingView` / `FamilyLinkView` / `PersonFamilyLinkView` などに分散し、`bordered`・即時解除・別ダイアログが混在しているため旧単一路線前提は対象外

### オフラインキュー順序保証（v12追加）
- [x] 同一 eventID のエントリが複数存在する場合、最新 timestamp のみが送信されること
- [x] キューがシリアル処理（並列送信なし）で実行されること
- [x] オフライン中に complete → skip の2操作が発生した場合、skip のみが送信されること
...
- 6桁コード経由の同意確認画面にも同じポジティブフレーミングが適用されること
  注: 現行コードベースではこの同意確認画面自体をまだ確認できていないため対象外

### AlarmKit権限プリプロンプト（v12追加）
- MagicDemoViewの前に AlarmKit権限プリプロンプト画面が表示されること
  注: 現行実装は AlarmKit 専用プリプロンプトではなく `PermissionsCTAView` の通知→カレンダー2ステップ構成のため対象外
- プリプロンプトに「マナーモードでも鳴る」理由が説明されていること
  注: 現行実装では `PermissionsCTAView` 通知ステップに「マナーモードでも必ずお知らせするために『通知』を使います」があり、AlarmKit 専用画面前提は対象外
- 「アラームを許可する」→ AlarmManager権限リクエスト → 許可後MagicDemoViewに進むこと
  注: 現行実装は `permissions.requestNotification()` / `requestCalendar()` の順で進み、AlarmManager 専用導線は対象外
- 「あとで設定する」→ MagicDemoViewをスキップしてPersonHomeViewへ進むこと
  注: 現行実装の「あとで →」は `PermissionsCTAView` 内で次ステップまたは `OwlNamingView` へ進む構成で、旧 AlarmKit 専用スキップ導線は対象外
- 通知・カレンダーのプリプロンプトと同じデザインパターンが使われていること
  注: 現行は AlarmKit 専用プリプロンプト自体がなく、`PermissionsCTAView` に統合されているため対象外
...
- ハッシュタグ「#忘れ坊アラーム #ADHD #できた」がシェアテキストに含まれること
  注: 現行コードベースでは月次サマリー / ShareLink / シェア画像生成の実装自体を確認できず、旧 Phase F 前提のため対象外

### 箱庭ウィジェット（v13追加）
- [x] Medium Widget が `.systemMedium` WidgetFamily で定義されていること
- [x] 左ペインにふくろうの部屋（1/3幅）、右ペインに予定情報（2/3幅）が配置されること
- [x] XP 0〜99: 部屋にふくろうのみ表示されること
...
- owlState が部屋の雰囲気に反映されること（sleepy→暗い / happy→明るい）
  注: 現行ウィジェットの部屋表現は `owl_xp` と残り時間ベースで構成され、旧 `owlState` 保存値を直接反映する仕様ではないため対象外
- [x] App Group UserDefaults 経由で owlXP が正しく読み取られること
- [x] WidgetCenter.shared.reloadAllTimelines がXP更新時に呼ばれること
...
- [s] `Color.white` / `Color(uiColor: .systemBackground)` が背景に直接使われていないこと
  提案: 全面禁止ではなく「画面の主要背景面」と「カード外周面」を `Color.Theme` / material / semantic background に寄せる対象へ絞ると、装飾ハイライトやアイコン塗りを巻き込まず段階的に整理しやすい
  対象範囲メモ: このラウンドでは `Settings` 系 / `Family` 系 / `RingingView` を優先し、`PersonHomeView` / `TimeOfDayBackground` は非対象とする
...
- [x] タップで全件が `.easeInOut(0.3)` アニメーションで展開されること
- [x] 展開後に「▲ 折りたたむ」ボタンが表示されること
- [x] 未完了が3件以下の場合、折りたたみボタンが表示されないこと（全件表示）
- [x] 完了済み予定はカウントに含まれないこと（完了後に3件未満になったら自動展開）
- [x] `@SceneStorage("isEventListExpanded")` で展開状態が再起動後も維持されること
  注: 現行実装の展開状態は `PersonHomeViewModel.isEventListExpanded` のみで、`@SceneStorage("isEventListExpanded")` は確認できていない
...
- 無料版では先月比較が🔒表示されること
  注: 現行コードベースでは家族向け「今月のまとめ」カードや先月比較UI自体をまだ確認できず、旧継続率施策前提のため対象外
...
- チェックイン画面から「続けて使う」タップで画面が閉じるだけで追加の引き止めがないこと
  注: 現行の `PaywallView` / 設定画面には専用の解約チェックイン導線が見当たらず、旧解約フロー前提は対象外
...
- ボタンが `.statusDanger` 色のテキストスタイル（赤い目立つボタンではないこと）
  注: 現行実装は `Button(role: .destructive)` + `Label` で、`.statusDanger` テキスト指定そのものではない
...
- ローカル UserDefaults が全クリアされること
  注: 現仕様では予定データ・利用設定を保持したまま家族連携/認証状態のみ外す方針のため対象外
- [r] AlarmKit の登録済みアラームがすべてキャンセルされること
- EventKit のアプリ作成イベント（wasure-bou マーカー付き）が削除されること
  注: 現仕様ではローカルの予定データを保持するため、EventKit 側の予定削除までは行わない
- .caf ファイルがすべて削除されること
  注: 現仕様ではローカルの予定データを保持するため、予定に紐づく音声ファイルも維持する前提
- [x] 処理中に ProgressView + 「削除中...」オーバーレイが表示されること（Dismiss不可）
- [2] Supabase失敗時にエラー表示 + ローカルデータは保持されること（再試行のため）
- [x] 削除完了後: オンボーディング画面に遷移すること
...
- 現行仕様では Hapticのみデモ導線は廃止し、「あとで試す」で `WidgetGuideView` へ進むため対象外

#### missed → complete 後からの手動完了（v15追加）
- [x] EventRow 長押しメニューで `missed` 状態の予定に「完了にする」が選べること
- [x] 選択後: completionStatus が `.complete` に更新されること
- [2] Supabase に dismissed_status = "complete" が PATCH されること（missed の上書き）
- [x] XP +10 が付与されること
- [x] EventRow の表示が ⏰グレーアウト → ✓完了グレーアウトに変わること

#### 折りたたみアニメ・マイク波形の減動作対応（v15追加・項目10）
- [x] `isReduceMotionEnabled = true` の時: EventRow 折りたたみが `.move` でなく `.opacity` のみになること
...
- `.presentationDetents([.medium, .large], selection: $binding)` の selection binding が使われていること
  注: 現行の手入力/マイク入力シートは `.presentationDetents([.large])` または固定 detent で、selection binding は未使用のため旧仕様前提は対象外

#### 既存ユーザーのオンボーディングスキップ（v15追加・矛盾1）
- [x] `isOnboardingComplete == true` かつ `appMode == nil` の場合: ModeSelection のみ表示されること
- [x] ModeSelection 完了後: 権限・OwlNaming・MagicDemo をスキップして直接 Home へ遷移すること
- [x] 既存ユーザーの ModeSelection ボタンラベルが「この設定で使う」であること（「はじめる」ではないこと）

#### オフラインキュー上限（v15追加・項目3）
- [x] キューが 100 件を超えた場合、最古エントリから順に破棄されること
- [x] キューサイズがデバッグビルドでログ出力されること
  注: 現行コードベースでは `OfflineActionQueue` 本体や上限管理の実装をまだ確認できていない
...
- フェッチ失敗時: バンドル JSON にフォールバックすること
  注: 現行実装は `VoiceFileGenerator.pronunciationMap` の静的定数辞書で、バンドル JSON / リモートフェッチ構成はまだ確認できていない

### v16 追加項目

#### DismissSheet（v16 P-2-1・RingingView 完了後ハーフシート）
- RingingView 完了後: DismissSheet（ハーフシート）が PersonHome 直前に表示されること
  注: 現行実装は `RingingView` 完了後に Toast と `PersonHomeView` 側の undo バナーへ寄っており、専用 `DismissSheet` は対象外
- DismissSheet に「✓ 完了しました！」+ XPアニメーション + 「取り消す」ボタンがあること
  注: 現行実装は `DismissSheet` ではなく Toast と `undoBanner` / `completeUndoBanner` の構成へ置き換わっているため対象外
- [2] 「取り消す」タップ: completionStatus が nil に戻り Supabase送信がキャンセルされること
- 10秒後に DismissSheet が自動閉じすること
  注: 現行実装は `DismissSheet` を使用せず `undoBanner` / `completeUndoBanner` ベースのため対象外
...
- [x] Toast が `.fullScreenCover`（RingingView）の上層に表示されること（裏に隠れないこと）
- [x] `UIWindow.windowLevel = .alert + 1` で実装されていること
...
- [x] `denied` 時: PersonHome 操作がブロックされ全画面警告が表示されること
- [x] 設定アプリへのディープリンクボタンが表示されること
...
- DismissSheet 30秒タスク確定後に `undoPendingUntil = nil` がセットされること
  注: 現行実装は `DismissSheet` を介さない完了/Undoフローへ置き換わっており、この旧ハーフシート前提の確認項目は対象外

#### スヌーズの家族ダッシュボード表示（P-9-2）
- [2] 家族Tab 0 に `snoozed` ステータス（🟡オレンジ・⏱アイコン）が表示されること
- スヌーズ中のテキストが「○分後にまたお知らせ中」と表示されること
  注: 現行実装は `RingingView` のボタンラベル/メッセージ中心で、家族ダッシュボード側にこの文言はまだ確認できていない
- [2] スヌーズ時に家族への Push 通知が送信されないこと
- スヌーズ3回目以降: `snoozed_count` が表示されること
  注: 現行実装は `snoozeCount` フィールドと文言分岐はあるが、`snoozed_count` という表示名やDB列ベースのUIは確認できていない

#### バッテリー死対応（P-9-3）
- [x] バッテリー10%未満 + 2時間以内のアラームがある場合にトーストが表示されること
- [x] `lowBattery` 条件でアニメーションが削減されること（`reduceMotion` と同等）
  注: 現行コードベースでは低バッテリー時のトーストはあるが、アニメーション削減分岐はまだ確認できていない
...
- セッション期限5分前に `refreshSession()` が呼ばれること
  注: 現行 `FamilyRemoteService` には `ensureValidSession()` / `refreshSession()` はなく、各所で `client.auth.session` 取得または `ensureDeviceRegistered()` を使う構成のため旧 API 名ベースは対象外
- [x] セッションなし時に `signInAnonymously()` が呼ばれること

#### データマイグレーション（P-9-6）
- `DataMigrationService.migrateIfNeeded()` が `ADHDAlarmApp.init()` の最初に呼ばれること
  注: 現行実装では `ADHDAlarmApp.init()` ではなく `AppDelegate.didFinishLaunchingWithOptions` の先頭付近で呼ばれているため旧配置前提は対象外
...
- 通話終了後に `recoverStaleAlarmState()` が呼ばれること
  注: 現行コードベースでは `CXCallObserverDelegate` / `recoverStaleAlarmState()` の実装をまだ確認できず、現仕様の明示実装対象にも見当たらないため対象外
...
- 無料/PRO境界の判定ロジックに `isToday` / `within7Days` / `olderThan7Days` の3分岐があること
  注: 現行コードベースではこの3関数名や同等の境界ロジックをまだ確認できず、旧無料/PRO出し分け設計前提は対象外

#### Android 対応方針の明記（P-9-11）
- FamilyPairingOnboardingView に「現在iPhoneのみ対応」の注記が表示されること
- 招待リンク生成時に「iPhoneをお使いの方のみ」のガイドが表示されること
  注: 現行コードベースに `FamilyPairingOnboardingView` や該当ガイド文言は見当たらず、旧オンボーディング前提のため対象外
...
- DismissSheet が3秒後（または「閉じる」タップ）で即座に閉じること（10秒ブロックしないこと）
  注: 現行実装は `DismissSheet` 自体がなく、`undoBanner` / `completeUndoBanner` 構成へ移行しているため対象外
- PersonHomeView の EventRow 上部に「↩ 取り消す（○秒）」リンクが30秒間表示されること
  注: 現行実装は EventRow 上部のカウントダウンリンクではなく、`undoBanner` / `completeUndoBanner` の「もとに戻す」ボタン表示
...
- `undoDeadline` タイマーがバックグラウンドでも継続することを確認すること
  注: 現行実装は `undoDeadline` という公開タイマー状態を持たず、`RingingViewModel.undoTask` による猶予処理へ移行しているため旧タイマー名ベースの確認は対象外

#### ToDo 翌日持ち越しルール（P-9-14）
- [x] `isToDo == true` かつ `completionStatus == nil` の予定が翌日に持ち越されること
- 持ち越し予定に `isCarriedOver == true` フラグがセットされること
  注: 現行実装は `isCarriedOver` 永続フラグではなく、`fireDate` が昨日以前かどうかを `EventRow` 側で導出してバッジ表示している
...
### データ同期・競合防止（追加項目）
- [x] scenePhase == .active 時: `OfflineActionQueue.shared.flush()` が `await` で完全終了してから `SyncEngine.performFullSync()` が開始されること
- [x] オフライン中に complete 操作がキューに積まれた状態でフル同期を走らせても、ローカルの完了状態が上書きされないこと
  注: 現行コードベースでは `OfflineActionQueue` 実装をまだ確認できていない
...
- [x] 購入後: `subscriptionTier == .pro` になり、PRO機能（カレンダー選択・複数事前通知等）が解放されること
- [x] 「あとで」: ペイウォールで「あとで」をタップしてもアプリが継続使用できること
- [x] 無料→PRO: 無料版でロック機能（履歴・SOS）にタップするとペイウォールが出ること
- [2] レビュー依頼タイミング: 完了10回到達後、または家族に初回✓✓が届いた後にのみレビューダイアログが出ること
- [x] レビュー依頼ブロック: 起動直後・エラー後・スキップ後・ペイウォール表示直後にレビューダイアログが出ないこと
...
- [x] アカウント削除: 完了後に `appState.familyLinkId = nil` がセットされること
- [x] アカウント削除: ローカルの予定データ（AlarmEventStore）は保持されること
- [x] アカウント削除: 削除中に `ProgressView + 「削除中...」` オーバーレイが表示され、途中でシートを閉じられないこと
- [x] アカウント削除: 完了後にオンボーディング画面（ModeSelectionView）に遷移すること
...
### 新規オンボーディング画面（追加項目）
- [x] PersonWelcomeView: ふくろうの羽ばたきアニメーションが表示されること
- PersonWelcomeView: [はじめる] ボタン以外のUI要素がないこと
  注: 現行実装には画像・3行コピーもあり、この旧単画面前提は対象外
- NotificationAuthView・CalendarAuthView: 権限ごとに独立した1画面として表示されること
  注: 現行実装は `NotificationAuthView` / `CalendarAuthView` ではなく `PermissionsCTAView` の2ステップ構成のため対象外
- FamilyPainQuestionView: 1つ以上選択するまで [次へ] が非活性であること
  注: 現行コードベースに `FamilyPainQuestionView` は見当たらず、旧オンボーディング前提のため対象外
- FamilySolutionView: FamilyPainQuestionView の回答内容に応じてメリットテキストが出し分けられること
  注: 現行コードベースに `FamilySolutionView` は見当たらず、旧オンボーディング前提のため対象外
- FamilyPairingIntroView: ❌マークが1箇所も使われていないこと（🛡️マークで代替）
  注: 現行コードベースに `FamilyPairingIntroView` は見当たらず、旧オンボーディング前提のため対象外
  注: 現行コードベースに `FamilyPairingIntroView` は見当たらず、家族導線は `FamilyPairingView` 中心の構成
...
- 登録者が自分でない場合、「✏️ 予定を編集する」ボタンが非表示であること
  注: 現行コードベースに `AlarmDetailView` は見当たらず、家族予定の詳細表示は `FamilyDashboardTab` のステータスチップ構成に寄っている
- 「この予定を削除する」タップで ActionSheet の2段階確認が表示されること
  注: 現行コードベースに `AlarmDetailView` は見当たらず、旧詳細ビュー前提のため対象外

### EditEventView（追加項目）
- EditEventView が PersonManualInputView と同じコンポーネントを再利用していること（新規フォームが別途作られていないこと）
- 既存イベントのタイトル・日時・繰り返し設定が初期値として表示されること
  注: 現行コードベースに `EditEventView` は見当たらず、旧編集画面前提のため対象外
...
- [x] 「あとでやる」でスキップ可能なこと
- 設定画面から再閲覧できること
  注: 現行コードベースではオンボーディング中の遷移はあるが、設定画面から `WidgetGuideView` を再表示する導線はまだ確認できていない
...
- [x] 1日1回の制限が機能すること（翌日リセット）
  注: 現行デイリーミニタスクは `UserDefaults` の完了日付 + Toast表示で管理されており、`.owlBounce` や完了後ボタン文言の専用状態遷移は確認できていない
...
- [x] @Observable ViewModel のプロパティ更新が MainActor 上で行われていること
  注記: `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` により、プロジェクト既定で MainActor 隔離が有効
- [x] バックグラウンドからUIへの反映で `await MainActor.run { }` が使われていること
- [x] Xcode の strict concurrency チェックでエラー・警告がゼロであること
  注記: 2026-04-07 時点で `xcodebuild -scheme ADHDAlarmTests test` のログから `warning:` は検出されなかった。なお、別件の既存テスト失敗は残っている

### Anonymous認証昇格（追加項目）
- [s] 匿名ユーザーが Apple ID と連携できる「Apple IDと連携する」ボタンが設定画面に表示されること（匿名ユーザーのみ）
  注: 現行コードベースでは Apple ID 連携ボタンや匿名→Apple昇格UIをまだ確認できていない
  提案: `SettingsView` にボタンを足す前に、匿名判定・`AuthenticationServices` の Apple認証・Supabase user_id移行API の3点を先に固める。UIだけ先行追加は避ける
...
- クラッシュレポートを Xcode Organizer で確認していること
  注: これはアプリ内仕様ではなく配布後の運用チェックのため、コードベース検証チェックからは対象外
- [r] テスター10名以上から「AlarmKitが実際に鳴った」フィードバックを収集していること
- フィードバックで指摘された文言・UIの修正が完了していること
  注: これは運用フェーズの継続タスクであり、静的な実装チェック項目としては対象外
